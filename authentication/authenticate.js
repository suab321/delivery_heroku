...
const sendOTP_R=(email,number)=>{
    const mailoption={
        ...
        html:`<p>Dear Customer</p><br><p>Thank Y for ordering from Stowaway.This is the OTP you would share with your service provider when they come to deliver the package.Without this OTP you wouldn't be able to complete the delivery.<h3>${number}</h3>*Do not share the OTP with anyone</p>`
    }
    transporter.sendMail(mailoption,(err,res)=>{
        if(err)
            console.log(err);

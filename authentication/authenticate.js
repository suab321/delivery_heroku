...
const sendOTP=(email,number,who)=>{
    ...
    const mailoption={
        ...
        html:`<p>Dear Customer</p><br><p>Thank you for orddering from Stowaway This is the OTP you would share with your service provider when they come to deliver the package to deliver the package.Without this OTP you wouldn't be able to complete the delivery.<h3>${number}</h3>*Do not share the OTP with anyone</p>`
    }
    transporter.sendMail(mailoption,(err,res)=>{
        if(err)
            console.log(err);
...
const verfiy=(email,token,Name)=>{
    const mailoption={
        ...
        html:'<h3> Hi'+Name+', </h3><p>Thank You and welcome to Stowaway.To activate your activate your account please click on the link below to confirm your registration.</p><p><a href="https://floating-brushlands-52313.herokuapp.com/authentication/verification/'+token+'">'+token+'</a></p><p>If you are having trouble clicking the link,copy and paste the URL above in your web browser</p><p>From,<br>Team Stowaway</p>'
    }
